...
Thread, Interrupt, -1
	...
	if (failed1 = 1){
		...
		if(MenuDisabledX > 0 and imonmenu = 0){
		...
		if(MenuDisabledX > 0 and otheronmenu =1 and imonmenu =1){
			...
			;Gosub, SenderText
			Sleep 1000
			WriteLog("menu start enabled: unpause then go init variables")						
			Gosub, dounpause
			Sleep 3000
			init()
		}
		
		;ImageSearch, MenuEnabledX, MenuEnabledY, 115, 560, 355, 595, *50 %A_ScriptDir%\pngs\menu_start_enabled.png
		;if(MenuEnabledX > 0){
     	;	WriteLog("menu start enabled found: send startros")
		;	StringSend := "startros"
		;	Gosub, SenderText
		;	WriteLog("menu start enabled: sleep 4000")
		;	Sleep 4000
		;	WriteLog("menu start enabled: block off")
		;	Gosub, DoUnBlockInput	
		;	Sleep 2000
		...
		;	ImageSearch , LeaveGame1X, LeaveGame1Y, leavegame1[1], leavegame1[2], leavegame1[3], leavegame1[4], *35 *TransBlack %scale% %pathpng%
		;	ImageSearch , LeaveGame2X, LeaveGame2Y, leavegame2[1], leavegame2[2], leavegame2[3], leavegame2[4], *35 *TransBlack %scale% %pathpng%
		;	ImageSearch , LeaveGame3X, LeaveGame3Y, leavegame3[1], leavegame3[2], leavegame3[3], leavegame3[4], *35 *TransBlack %scale% %pathpng%
		;	ImageSearch , LeaveGame4X, LeaveGame4Y, leavegame4[1], leavegame4[2], leavegame4[3], leavegame4[4], *35 *TransBlack %scale% %pathpng%
		;	ImageSearch , LeaveGame5X, LeaveGame5Y, leavegame5[1], leavegame5[2], leavegame5[3], leavegame5[4], *35 *TransBlack %scale% %pathpng%
		;	if(LeaveGame1X > 0 and LeaveGame2X > 0 and LeaveGame3X > 0 and LeaveGame4X > 0 and LeaveGame5X > 0 ){
		;	WriteLog("imagereader logout on esc blocked")
		;		Gosub, DoUnBlockInput
		;		Sleep 500
		;		SendInput, {Esc}				
		;		Sleep 500
	...
	;;accept omni
		...
		if (paused = 1){
			...
			if(AcceptRiftX > 0){
			  WriteLog("imagereader AcceptGr omni")		
			  Loop, 200 {
			  ImageSearch , AcceptRiftX, AcceptRiftY, acceptgr[1], acceptgr[2], acceptgr[3], acceptgr[4], *50 %scale% %pathpng%
				  if (AcceptRiftX > 0){
					c1 := acceptclick[1]
					c2 := acceptclick[2]
					Click %c1% , %c2%
					WriteLog("accept omni:clicking accept")
					entered := 1
					exited := 0
					wentback := 0
					uiclosed := 0
	...
	;		}
	;		WriteLog("send riftacceptbutton")
	;		StringSend := "riftacceptbutton"
	;		Gosub, SenderText
	;	}
	;}
